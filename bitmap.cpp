...
#include<iostream>
#include<windows.h>
#define DEBUG 1 //set true to show debug information
class Bitmap {
	public:
		char name[50];
		int height;
...
class BMP256 : public Bitmap {
	public:
		...
		BMP256(int row, int column) {
			width = column;
			height = row;
			int dataSizePerLine = ((column + 3) / 4) * 4;
...
class BMP24 : public Bitmap {
	public:
		...
		BMP24(const char* file) {
			std::ifstream inBmp(file, std::ios::in | std::ios::binary);
			if (!inBmp) {
				std::cout << "Read file error!" << std::endl;
				throw 0;
			}
			if(DEBUG) std::cout << "Reading from" << file << std::endl;

			inBmp.read((char*)&fileHead, sizeof(fileHead));
			inBmp.read((char*)&bmpInfoHead, sizeof(bmpInfoHead));
			if (bmpInfoHead.biBitCount != 24) {
				std::cout << "Not a 24 bit bitmap!" << std::endl;
				throw 0;
			}
			height = bmpInfoHead.biHeight;
			width = bmpInfoHead.biWidth;
			...
			px = new RGB*[height];
			for (int i = 0; i < height; i++) px[i] = new RGB[width];
		...
		BMP24(int row, int column) {
			width = column;
			height = row;
			int dataSizePerLine = ((column * 3 + 3) / 4) * 4;
			int fileSize = row * dataSizePerLine + 54;

			fileHead.bfType = 19778;//short		"BM"
			fileHead.bfSize = fileSize;//long
			fileHead.bfReserved1 = 0;//short
			fileHead.bfReserved2 = 0;//short
			fileHead.bfOffBits = 54;//long	0036h
			
			bmpInfoHead.biSize = sizeof(bmpInfoHead);//long
			bmpInfoHead.biWidth = width;//long
			bmpInfoHead.biHeight = height;//long
			bmpInfoHead.biPlanes = 1;//short
			bmpInfoHead.biBitCount = 24;//short
			bmpInfoHead.biCompression = 0;//long
			bmpInfoHead.biSizeImage = fileSize - 54;//long
			bmpInfoHead.biXPelsPerMeter = 0;//long
			bmpInfoHead.biYPelsPerMeter = 0;//long
			bmpInfoHead.biClrUsed = 0;//long
			bmpInfoHead.biClrImportant = 0;//long
		...
		BMP24(BMP256* b) {
			...
			this->height = b->height;
			int dataSizePerLine = ((width * 3 + 3) / 4) * 4;
			int fileSize = height * dataSizePerLine + 54;
			...
			px = new RGB*[height];
			BMP256::Palette::Color *c = b->palette.color;
			for (int i = 0; i < height; i++) {
				px[i] = new RGB[width];
				for (int j = 0; j < width; j++) {
					px[i][j].R = c[b->px[i][j]].R;
					px[i][j].G = c[b->px[i][j]].G;
					px[i][j].B = c[b->px[i][j]].B;
				}
			}
		...
		void writeFile(const char* path = NULL) {
			int dataSizePerLine = (width * 3 + 3) / 4 * 4;
			int fileSize = bmpInfoHead.biHeight * dataSizePerLine + 54;
